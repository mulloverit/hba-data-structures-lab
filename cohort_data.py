...
def all_students_tuple_list(filename):
    ...
    #turn each line into split list
    #join first and last name from spit list
    #turn each list into tuple
    #append tuple to student_list
    #return student_list
    # list_name = [' '.join(line.split('|')[execution_index_1:execution_index_2]) for line in cohort if line.split('|')[conditional_index].startswith(cohort_season)]
    ...
    for line in cohort:
        ...
        student_attributes = line.rstrip().split('|')
        
        if student_attributes[-1] not in ['G', 'I']:
            full_name = ' '.join(student_attributes[0:2])
